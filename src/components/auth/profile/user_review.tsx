...
const UserReview = (comm :DocumentData) => {
    ...
    useEffect(() => {
        if (!queryCinema) {
            get("cinemas", review["cinema_id"]).then((docSnap) => {
                if(docSnap) setQueryCinema({...docSnap, id: review["cinema_id"]});
            })
            ...
            >   
                ...
                <Box marginLeft='2rem' display='flex' flexDirection='column' width='100%' justifyContent='center'>
                        ...
                        <Box display='flex' flexDirection='row' justifyContent='space-between' alignItems='center'>
                            <Box display='flex' flexDirection='column' justifyContent='center' alignItems='center' gap='1rem'>
                                ...
                                <Box display='flex' flexDirection='row' gap='1ch'>
                                    ...
                                    <Typography fontStyle='italic' color={theme.palette.mode==='dark' ? 'white' : colors.blueAccent[400]}>{`${parseDate(review["created_date"])}`}</Typography>
                                </Box>
                            </Box>
                        </Box>
                    </Box>
                    ...
                    <Box sx={{ borderRadius:'15px', width:'100%', height:'auto', backgroundColor:theme.palette.mode === 'dark' ? '#353535' : '#cccccc', padding:'20px'}} >
                        <Box display='flex' flexDirection='row' justifyContent='space-between' alignItems='center' gap='5ch'>
                            <Rating
                                sx={{
                                    fontSize: '30px'
                                }}
                            ...
                            <Typography 
                                ...
                                fontStyle='italic'
                                >
                                {review["review_stars"]} star{review["review_stars"] === 1 ? "" : "s"} rating
                            </Typography>
                        </Box>
                    </Box>
                </Box>
